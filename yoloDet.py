...
class YoloTRT():
    def __init__(self, library, engine, conf, yolo_ver):
        ...
        self.categories = ["Dummy", "Korban", "Turu"]
        self.class_colors = {
            "Dummy": [0, 125, 0],      
            "Korban": [0, 0, 200],   
            "Turu": [150, 150, 0],   
        ...
        self.batch_size = 1 #self.engine.max_batch_size
    ...
    def PostProcess(self, output, origin_h, origin_w):
        ...
        # Batasi jumlah bounding box menjadi maksimal 3
        if len(boxes) > 3:
            boxes = boxes[:3]
    ...
    def PlotBbox(self, x, img, color=None, label=None, line_thickness=None):
        ...
        # Gunakan warna dari self.class_colors jika label ada
        if label:
           color = self.class_colors.get(label.split(":")[0], [random.randint(0, 255) for _ in range(3)])
